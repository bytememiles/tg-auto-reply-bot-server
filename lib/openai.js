...
function buildSystemPrompt(category, isNoisyUser) {
  const base =
    "You are a Telegram group bot. Reply in one short sentence only. ";
  const strongTone = isNoisyUser
    ? " This user is a known channel breaker/noisy user: use a stronger, harsher, more direct and mocking tone. "
    : "";
  const norule =
  ...
  return base + strongTone + "Max 15 words. " + norule;
}
 ...
 * @param {boolean} isNoisyUser - use stronger, harsher tone when true
 * @returns {Promise<string>} - reply text or "NOREPLY"
 */
async function getReply(category, currentMessage, isNoisyUser) {
  ...
  const systemContent = buildSystemPrompt(category, !!isNoisyUser);
...
function buildBotIdentityPrompt(isNoisyUser) {
  const base =
  ...
  const strongTone = isNoisyUser
    ? "This user is a known channel breaker/noisy user: use a stronger, more direct and dismissive tone. "
    : "";
  return (
    base +
    strongTone +
 ...
 * @param {string} currentMessage
 * @param {boolean} isNoisyUser - use stronger tone when true
 * @returns {Promise<string>} - reply text or "NOREPLY"
 */
async function getReplyBotIdentity(currentMessage, isNoisyUser) {
  const apiKey = process.env.OPENAI_API_KEY;
  if (!apiKey) {
    throw new Error("OPENAI_API_KEY is not set");
  }
  ...
  const response = await client.chat.completions.create({
    model: OPENAI_MODEL,
    messages: [
      { role: "system", content: buildBotIdentityPrompt(!!isNoisyUser) },
